...
void ir_send_command(const char* saved_device_uuid,
  ...
  for (size_t i = 0; i < mqttFeatures.size(); i++) {
    ...
    if (strcmp(featureNameval, "mode") == 0) {
      switch (cmd) {
        case kCoolixCool: Serial.println("ir_send_command - setting mode to Cool"); ac.setMode(kCoolixCool); break;
        case kCoolixDry:  Serial.println("ir_send_command - setting mode to Dry");  ac.setMode(kCoolixDry);  break;
        case kCoolixAuto: Serial.println("ir_send_command - setting mode to Auto"); ac.setMode(kCoolixAuto); break;
        case kCoolixHeat: Serial.println("ir_send_command - setting mode to Heat"); ac.setMode(kCoolixHeat); break;
        case kCoolixFan:  Serial.println("ir_send_command - setting mode to Fan");  ac.setMode(kCoolixFan);  break;
        default: Serial.println("ir_send_command - cannot set mode. Unsupported value!"); break;
      }
    }
    if (strcmp(featureNameval, "fanSpeed") == 0) {
      switch (cmd) {
        case kCoolixFanAuto0: Serial.println("ir_send_command - setting fan speed to Auto0"); ac.setFan(kCoolixFanAuto0); break;
        case kCoolixFanMax:   Serial.println("ir_send_command - setting fan speed to Max");   ac.setFan(kCoolixFanMax);   break;
        case kCoolixFanMed:   Serial.println("ir_send_command - setting fan speed to Med");   ac.setFan(kCoolixFanMed);   break;
        case kCoolixFanMin:   Serial.println("ir_send_command - setting fan speed to Min");   ac.setFan(kCoolixFanMin);   break;
        case kCoolixFanAuto:  Serial.println("ir_send_command - setting fan speed to Auto");  ac.setFan(kCoolixFanAuto);  break;
