...
TEST_F(DisplayTest, ButtonPressRotatesThroughControllerFeatureValuesAndWraps) {
  initDisplayForUpdate();
  addFeatureValue(0, "on", "", 1.0f, true);
  ...
  EXPECT_EQ(DisplayMockState::instance().prints[1], "On");
  ...
  EXPECT_EQ(DisplayMockState::instance().prints[7], "On");
}

TEST_F(DisplayTest, ButtonPressMapsBekoFanSpeedValuesToLabels) {
  initDisplayForUpdate();
  addFeatureValue(0, "fanSpeed", "-", 0.0f, true);
  addFeatureValue(1, "fanSpeed", "-", 1.0f, true);
  addFeatureValue(2, "fanSpeed", "-", 4.0f, true);
  ...
  EXPECT_EQ(DisplayMockState::instance().prints[1], "Auto0");
  EXPECT_EQ(DisplayMockState::instance().prints[2], "fanSpeed");
  EXPECT_EQ(DisplayMockState::instance().prints[3], "Max");
  EXPECT_EQ(DisplayMockState::instance().prints[4], "fanSpeed");
  EXPECT_EQ(DisplayMockState::instance().prints[5], "Min");
}
