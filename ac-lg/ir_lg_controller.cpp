...
void ir_send_command(const char* saved_device_uuid,
  ...
  for (size_t i = 0; i < mqttFeatures.size(); i++) {
    ...
    if (strcmp(featureNameval, "mode") == 0) {
      switch (cmd) {
        case kLgAcCool: Serial.println("ir_send_command - setting mode to Cool"); ac.setMode(kLgAcCool); break;
        case kLgAcDry:  Serial.println("ir_send_command - setting mode to Dry");  ac.setMode(kLgAcDry);  break;
        case kLgAcFan:  Serial.println("ir_send_command - setting mode to Fan");  ac.setMode(kLgAcFan);  break;
        case kLgAcAuto: Serial.println("ir_send_command - setting mode to Auto"); ac.setMode(kLgAcAuto); break;
        case kLgAcHeat: Serial.println("ir_send_command - setting mode to Heat"); ac.setMode(kLgAcHeat); break;
        default: Serial.println("ir_send_command - cannot set mode. Unsupported value!"); break;
      }
    }
    if (strcmp(featureNameval, "fanSpeed") == 0) {
      switch (cmd) {
        case kLgAcFanHigh:   Serial.println("ir_send_command - setting fan speed to Max");  ac.setFan(kLgAcFanHigh);   break;
        case kLgAcFanMedium: Serial.println("ir_send_command - setting fan speed to Med");  ac.setFan(kLgAcFanMedium); break;
        case kLgAcFanLowest: Serial.println("ir_send_command - setting fan speed to Min");  ac.setFan(kLgAcFanLowest); break;
        case kLgAcFanAuto:   Serial.println("ir_send_command - setting fan speed to Auto"); ac.setFan(kLgAcFanAuto);   break;
