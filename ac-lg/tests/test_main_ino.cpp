...
TEST_F(MainInoTest, BuildFeaturesIncludesAdmissionSpecs) {
  ...
  EXPECT_FLOAT_EQ(setpointSpec["min"].as<float>(), 16.0f);
  ...
  EXPECT_STREQ(modeList[2]["text"].as<const char*>(), "Fan");
  EXPECT_EQ(modeList[3]["value"].as<int>(), 3);
  EXPECT_STREQ(modeList[3]["text"].as<const char*>(), "Auto");
  EXPECT_EQ(modeList[4]["value"].as<int>(), 4);
  EXPECT_STREQ(modeList[4]["text"].as<const char*>(), "Heat");
  ...
  ASSERT_EQ(fanSpeedList.size(), 4u);
  EXPECT_EQ(fanSpeedList[0]["value"].as<int>(), 10);
  EXPECT_STREQ(fanSpeedList[0]["text"].as<const char*>(), "Max");
  EXPECT_EQ(fanSpeedList[1]["value"].as<int>(), 2);
  EXPECT_STREQ(fanSpeedList[1]["text"].as<const char*>(), "Med");
  EXPECT_EQ(fanSpeedList[2]["value"].as<int>(), 0);
...
// Forward-declare mqtt_callback (defined in ac-lg.ino, no header).
...
TEST_F(MainInoTest, MqttCallbackShowsCommandReceivedMessage) {
  ...
  const char* payload = R"([
    ...
    {"featureName":"setpoint","payload":{"value":22}}
  ])";
  auto* p = reinterpret_cast<uint8_t*>(const_cast<char*>(payload));
