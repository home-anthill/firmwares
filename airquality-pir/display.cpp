...
static const char* airquality_display_label(float value) {
...
static const char* motion_display_label(float value) {
  int motion = static_cast<int>(value);
  if (motion == 1) {
    return "Detected";
  }
  if (motion == 0) {
...
static void render_message(const char* title, const char* detail) {
  render_two_line_screen(
      title == nullptr ? "Status" : title,
      detail == nullptr ? "Online" : detail);
...
static void render_feature_value(const FeatureValue& value) {
  ...
  char value_line[32];
  uint8_t value_text_size = 2;
  const char* label = nullptr;

  if (strcmp(value.name, "airquality") == 0) {
    label = airquality_display_label(value.value);
    if (label != nullptr && strlen(label) > 10) {
      value_text_size = 1;
    }
  } else if (strcmp(value.name, "motion") == 0) {
    label = motion_display_label(value.value);
  }
