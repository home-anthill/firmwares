...
TEST_F(DisplayTest, UpdateDisplayRefreshesVisibleAirQualityLabel) {
  initDisplayForUpdate();
  addFeatureValue(0, "airquality", "-", 2.0f, true);
  pressButtonFor(100);
  clearDisplayMockCalls();

  g_feature_values[0].value = 3.0f;
  update_display();

  ASSERT_GE(DisplayMockState::instance().prints.size(), 2u);
  EXPECT_EQ(DisplayMockState::instance().prints[0], "airquality");
  EXPECT_EQ(DisplayMockState::instance().prints[1], "Excellent");
}

TEST_F(DisplayTest, UpdateDisplayRefreshesVisibleMotionLabel) {
  initDisplayForUpdate();
  addFeatureValue(0, "motion", "-", 0.0f, true);
  pressButtonFor(100);
  clearDisplayMockCalls();

  g_feature_values[0].value = 1.0f;
  update_display();

  ASSERT_GE(DisplayMockState::instance().prints.size(), 2u);
  EXPECT_EQ(DisplayMockState::instance().prints[0], "motion");
  EXPECT_EQ(DisplayMockState::instance().prints[1], "Detected");
}
...
TEST_F(DisplayTest, ButtonPressRendersAirQualityLabels) {
  const struct {
    float value;
    const char* label;
  } cases[] = {
  ...
  for (const auto& test_case : cases) {
    SetUp();
    initDisplayForUpdate();
    addFeatureValue(0, "airquality", "-", test_case.value, true);

    pressButtonFor(100);

    ASSERT_GE(DisplayMockState::instance().prints.size(), 2u);
    EXPECT_EQ(DisplayMockState::instance().prints[0], "airquality");
    EXPECT_EQ(DisplayMockState::instance().prints[1], test_case.label);
  }
}

TEST_F(DisplayTest, ButtonPressRendersMotionLabels) {
  const struct {
    float value;
    const char* label;
  } cases[] = {
  ...
  for (const auto& test_case : cases) {
    SetUp();
    initDisplayForUpdate();
    addFeatureValue(0, "motion", "-", test_case.value, true);
