...
// and MODEL ("dht-light") as defined in secrets.h.
static std::string valid_response() {
  return R"({"uuid":"test-uuid-0000-0000-000000000000","mac":"aa:bb:cc:dd:ee:ff","manufacturer":"ks89","model":"dht-light","features":[{"uuid":"feat-uuid-0000-0000-000000000001","type":"temperature","name":"Temperature","enable":true,"order":1,"unit":"C"}]})";
}
...
TEST_F(RegistrationTest, OkResponseRegistersAndStoresData) {
  ...
  EXPECT_STREQ(arr[0]["type"].as<const char*>(), "temperature");
...
TEST_F(RegistrationTest, ReturnsThreeWhenRequiredFieldsMissingFromResponse) {
  ...
  HttpMockConfig::instance().response_body =
    R"({"mac":"aa:bb:cc:dd:ee:ff","manufacturer":"ks89","model":"dht-light","features":[]})";
  EXPECT_EQ(register_insecure_server(wifi_client, kMac, features_doc), 3);
  ...
  HttpMockConfig::instance().response_code = HTTP_CODE_OK;
  HttpMockConfig::instance().response_body =
    R"({"uuid":"test-uuid-0000-0000-000000000000","manufacturer":"ks89","model":"dht-light","features":[]})";
  EXPECT_EQ(register_insecure_server(wifi_client, kMac, features_doc), 3);
}
...
TEST_F(RegistrationTest, ReturnsFourOnResponseMismatch) {
  ...
  // mac mismatch
  HttpMockConfig::instance().response_body =
    R"({"uuid":"test-uuid-0000-0000-000000000000","mac":"ff:ff:ff:ff:ff:ff","manufacturer":"ks89","model":"dht-light","features":[]})";
  EXPECT_EQ(register_insecure_server(wifi_client, kMac, features_doc), 4);

  // manufacturer mismatch
  HttpMockConfig::instance().response_body =
    R"({"uuid":"test-uuid-0000-0000-000000000000","mac":"aa:bb:cc:dd:ee:ff","manufacturer":"wrong","model":"dht-light","features":[]})";
  EXPECT_EQ(register_insecure_server(wifi_client, kMac, features_doc), 4);
