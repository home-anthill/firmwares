...
static unsigned long connectivity_notice_until_ms = 0;
#ifdef HOME_ANTHILL_TEST_OLED_DISPLAY
...
static const unsigned long DISPLAY_CONNECTIVITY_NOTICE_MS = 10000;
...
static bool render_feature_value_by_name(const char* name) {
  if (!available || !powered || name == nullptr) {
    return false;
  }
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    if (feature_values_get(i, &value) &&
      ...
      render_state.next_feature_index = (i + 1) % count;
      render_state.current_feature_index = i;
      render_feature_value(value);
      return true;
    }
  }
...
static void render_default_feature_value() {
  if (!button_enabled() && render_feature_value_by_name("temperature")) {
    return;
  }
...
static void render_connectivity_fallback_feature_value() {
  if (render_feature_value_by_name("temperature")) {
    return;
  }
...
static void update_connectivity_error_display() {
  display_sleep_until_ms = 0;
  power_on();
  if (deadline_active(connectivity_notice_until_ms)) {
  ...
  if (render_state.current_error ||
      render_state.current_message ||
      !render_state.current_valid) {
    render_connectivity_fallback_feature_value();
  } else {
...
void init_display(uint8_t configured_display_button_pin) {
  ...
  display_sleep_until_ms = 0;
  connectivity_notice_until_ms = 0;
  powered = true;
  render_state.has_rendered = false;
  render_state.current_valid = false;
  render_state.current_error = false;
  render_state.current_message = false;
...
void display_set_connectivity_status(bool wifi_connected, bool mqtt_connected) {
  ...
  connectivity.mqtt_connected = mqtt_connected;
  bool has_error = connectivity_has_error();
  if (changed) {
    invalidate_render();
  }
  if (!has_error) {
    ...
    connectivity_notice_until_ms = millis() + DISPLAY_CONNECTIVITY_NOTICE_MS;
  }
}
...
void display_force_update() {
  ...
  power_on();
  if (connectivity_has_error() &&
      deadline_active(connectivity_notice_until_ms)) {
    render_connectivity_error();
    return;
  }
